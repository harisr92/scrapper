...
const HTTP_MAX_STATUS = 399
const DEFAULT_USER_AGENT = "Go Link Health (github.com/harisr92/go-link-health)"
...
func setHandlers(collector *colly.Collector) *colly.Collector {
	collector.OnError(func(response *colly.Response, err error) {
		...
		handleError(fmt.Errorf("Request to %s failes. Reason: %s", url, reason))
	})
